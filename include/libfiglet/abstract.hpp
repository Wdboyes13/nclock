...
namespace srilakshmikanthanp
{
  namespace libfiglet
  {
    ...
    protected:                                                            // Protected Methods
       ...
       * @param hb hardblank
       */
      fig_str_type rm_hardblank(const fig_str_type &figs) const
      {
        fig_str_type result = figs;
        for (size_type i = 0; i < result.size(); ++i)
        {
          std::replace(result[i].begin(), result[i].end(), this->hard_blank, traits_type::to_char_type(' '));
        }
        return result;
      }
